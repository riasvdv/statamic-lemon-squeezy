...
{
    protected $icon = '<svg width="20px" height="18px" viewBox="0 0 128 174" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    public function augment($value)
    {
        ...
        if ($value['overlay'] ?? false) {
            return <<<HTML
                ...
                <a class="lemonsqueezy-button" href="{$link}?embed=1">{$value['buttonText']}</a>
            HTML;
        }

        return <<<HTML
            <a href="{$link}">{$value['buttonText']}</a>
        HTML;
    }
}
